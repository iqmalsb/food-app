            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('organisation.update-settings') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            <label class="form-label">Default Theme Color</label>
                            <div class="d-flex gap-3 mt-1">
                                @foreach(['indigo' => '#6366f1', 'emerald' => '#10b981', 'blue' => '#3b82f6', 'rose' => '#f43f5e', 'orange' => '#f97316'] as $color => $hex)
                                    <div class="form-check form-check-inline m-0">
                                        <input class="form-check-input d-none" type="radio" name="theme_color" id="color_{{ $color }}" value="{{ $color }}" {{ old('theme_color', $organisation->theme_color) === $color ? 'checked' : '' }}>
                                        <label class="d-flex align-items-center justify-content-center rounded-circle" for="color_{{ $color }}" style="width: 38px; height: 38px; background-color: {{ $hex }}; cursor: pointer; border: 3px solid {{ old('theme_color', $organisation->theme_color) === $color ? 'var(--text-primary)' : 'transparent' }}; box-shadow: 0 0 5px rgba(0,0,0,0.1); transition: all 0.2s;" onclick="selectColor(this)">
                                            @if(old('theme_color', $organisation->theme_color) === $color)
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select @error('theme_mode') is-invalid @enderror" id="theme_mode" name="theme_mode" required>
                                <option value="dark" {{ old('theme_mode', $organisation->theme_mode) === 'dark' ? 'selected' : '' }}>🌙 Dark Mode (Default / Landing Page Scheme)</option>
                                <option value="light" {{ old('theme_mode', $organisation->theme_mode) === 'light' ? 'selected' : '' }}>☀️ Light Mode</option>
                            </select>
                            <div class="form-text">Choose the default color palette mode for all users under your organisation.</div>
                            @error('theme_mode')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
